...
switch ($path) {
    ...
    case preg_match('/^businesses\/(\d+)$/', $path, $matches) ? $path : '':
        $id = intval($matches[1]);
    ...
    case preg_match('/^reviews\/(\d+)$/', $path, $matches) ? $path : ''):
        $id = intval($matches[1]);
    ...
    case 'reviews-business/(\d+)':
        if (preg_match('/^reviews-business\/(\d+)$/', $path, $matches)) {
